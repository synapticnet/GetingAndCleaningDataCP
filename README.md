...
The output of this script will be a tidy data set as required by the coursera.org class "Getting and Cleaning Data". It loads in the data from the UCI data set linked to at the course project page.  It then extracts out the features that represent mean and standard deviations from the rest of the features.  Then these extracted features are then used to create a feature mean by activity for each subject. This data set will then be output as a txt file named "featMeanByActForSubject.txt" that may be read back into an R data frame by the command `read.table("featMeanByActForSubject.txt", header = TRUE)`
